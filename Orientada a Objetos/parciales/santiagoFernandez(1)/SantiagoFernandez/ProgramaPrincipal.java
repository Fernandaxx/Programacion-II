...
public class ProgramaPrincipal{
    ...
    public static void main (){
        ...
        System.out.println("Ingrese cuantas cuentas bancarias desea ingresar");
        int dimF = in.nextInt();
        cuentas = new CuentaBancaria[dimF]; 
        for (int i = 0; i < dimF; i++){
            cuentas [i] = ingresoCuenta(in);
        }
        for (int i = 0; i<dimF; i++){
            System.out.println(cuentas [i].toString());
            System.out.println("\n"+"El BNA percibira un impuesto por esta cuenta de: "+ cuentas[i].impuestos());
        }
    }
    ...
    private static CajaDeAhorro ingresoCDA (Scanner in, int numCuenta, int cbu, String alias, double dinero, Propietario propietario){
        ...
        CajaDeAhorro aux = new CajaDeAhorro(numCuenta, cbu, alias, dinero, propietario, plazoFijo, tarjetas);
